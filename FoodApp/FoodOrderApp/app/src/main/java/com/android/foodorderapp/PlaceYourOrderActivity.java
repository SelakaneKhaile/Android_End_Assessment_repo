...
public class PlaceYourOrderActivity extends AppCompatActivity {
    ...
    private void calculateTotalAmount(RestaurantModel restaurantModel) {
        ...
        tvSubtotalAmount.setText("M"+String.format("%.2f", subTotalAmount));
        if(isDeliveryOn) {
            tvDeliveryChargeAmount.setText("M"+String.format("%.2f", restaurantModel.getDelivery_charge()));
            subTotalAmount += restaurantModel.getDelivery_charge();
        }
        tvTotalAmount.setText("M"+String.format("%.2f", subTotalAmount));
    }
